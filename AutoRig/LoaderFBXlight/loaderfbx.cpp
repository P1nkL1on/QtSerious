...
QString loaderFBX::loadMeshOBJ(QTextStream &textStream, Mesh &loadedMesh)
{
    ...
    for (int i = 0; i < loadedVertexes.length(); i++)
        loadedDerV3s << Matrix<Derivable,1,3>(Derivable(3) * Derivable(loadedVertexes[i].x()) + Derivable(50),Derivable(3) * Derivable(loadedVertexes[i].y())  + Derivable(20),Derivable(3) * Derivable(loadedVertexes[i].z()));
