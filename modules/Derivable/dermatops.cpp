...
Eigen::Matrix<Derivable, 4, 4> DerivableVectorMatrixes::MakeDeriveScaleMatrix(const Matrix<Derivable,1,3> scale)
{
    Matrix<Derivable, 4, 4> M = SetDeriveMatrix();
    for (int i = 0; i < 3; i++)
        M(i,i) = scale(0,i);
...
void DerivableVectorMatrixes::ScaleDeriveMatrix(Eigen::Matrix<Derivable, 4, 4> &originalMatrix, const Matrix<Derivable,1,3> scale)
{
    originalMatrix = originalMatrix * MakeDeriveScaleMatrix(scale);
}
